  ...
  return {
        ...
        elements : [
          {
            ...
            onChange : function(){
              var select = this;
              var hselect = select.getInputElement().$;
              this.link_text = hselect.children[hselect.options.selectedIndex].innerText;
             var dialog = this.getDialog();
              var field = dialog.getContentElement( 'info', 'url' );
              field.setValue(window.location.host+"/"+select.getValue());
            },
            setup : function(data){
              var select = this;
              select.clear();
              select.add('Choisir une page', "");
              $.ajax('/admin/pages/list_pages.json',
               {
                type : "GET",
                async: false,
                success : function(data){
                  for(var i in data){ 
                    select.add(data[i].title, data[i].slug);
                  };
                }
              });
    ...
    onOk : function()
    {
      ...
      if ( !this._.selectedElement )
      {
        ...
        attributes[ 'data-cke-saved-text' ] = data['link_text'];
        ...
        if ( ranges.length == 1 && ranges[0].collapsed )
        {
          ...
          var text = new CKEDITOR.dom.text( data.type == 'email' ?
              data.email.address : attributes[ 'data-cke-saved-text' ], editor.document );
